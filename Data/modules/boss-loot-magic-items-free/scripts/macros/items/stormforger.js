...
const artWorkChatCardAnimated = `modules/${MODULE_NAME}/artwork/005-stormforger/art-animated-for-chat-stormforger-staff.gif`;
const artWorkChatCardStatic = `modules/${MODULE_NAME}/artwork/005-stormforger/art-static-stormforger-staff.webp`;
...
async function item({ speaker, actor, token, character, item, args, scope, workflow }) {
  ...
  thunderstormOfMiseryItem.flags['midi-qol'].onUseMacroName = '[preActiveEffects]function.bossLoot.macros.stormforger.thunderstormItem';
  ...
  const aerialAscensionAnimation = Sequencer.EffectManager.getEffects({ name: 'Aerial-Ascension', object: token });
  ...
  const abilitySelection = `<div class="dnd5e chat-card item-card midi-qol-item-card">
    <header class="card-header flexrow">
      <img src="${artWorkChatCardStatic}" title="${itemName}" width="36" height="36" />
...
async function itemAfterActiveEffects({ speaker, actor, token, character, item, args, scope, workflow }) {
  // await helpers.hideDivFromChatMessage(workflow.itemCardId, '#boss-loot-stormforger');
