...
async function failChainVisual(sourceToken, targetToken) {
  ...
  await new Sequence()
    //SHOOT
    .effect()
    .file('bossLoot.spell.range.snipe.burst.dust')
    .elevation(elevation)
    .filter('Glow', {
      outerStrength: 2,
      innerStrength: 0,
    ...
    //CHAIN
    .effect()
    .file('bossLoot.equipment.line.chain.metal')
    .attachTo(sourceToken)
...
async function successChainVisual(sourceToken, targetToken) {
  ...
  await new Sequence()
    ...
    //CHAIN_BIND
    .effect()
    .file('bossLoot.equipment.radial.chain.metal.spin.3')
    .elevation(elevation)
    .name('Chainhook-Bracer-Chain')
...
async function itemTemplatePlaced({ speaker, actor, token, character, item, args, scope, workflow }) {
  ...
  if (!isProperSize) {
    ...
    await helpers.createChatMessage('FAIL', textToChat, itemNameNormalized, artWorkChatCardStatic, artWorkChatCardAnimated);
