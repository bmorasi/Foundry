...
import { MANDATORY_MODULES, BOSS_LOOT_LOGO, HELPER_GIFS } from './constants.js';
import { log } from './boss-loot-log.js';

export const helperData = {
  replaceChatArtwork: async function _replaceChatArtwork(id, animatedArtwork = '', staticArtwork = '') {
    const animationSetting = game.settings.get(MODULE_NAME, 'itemAnimatedArtwork');
    ...
    switch (animationSetting) {
      case 0: // Enabled with animation
        if (animatedArtwork) {
          const existingAnimation = cardContent.querySelector('img');
          const animatedHTML = `<img src="${animatedArtwork}" width="272" height="272"/>`;

          if (existingAnimation) {
            existingAnimation.src = animatedArtwork;
          } else {
            cardContent.insertAdjacentHTML('beforeend', animatedHTML);
          }
        }
        break;

      case 1: // Enabled with static artwork
        if (staticArtwork) {
          const existingStaticImage = cardContent.querySelector('img');
          const staticHTML = `<img src="${staticArtwork}" width="272" height="272"/>`;

          // Check if the image needs to be updated or added
          if (existingStaticImage) {
            if (existingStaticImage.src !== staticArtwork) {
              existingStaticImage.src = staticArtwork;
            }
          } else {
            cardContent.insertAdjacentHTML('beforeend', staticHTML);
          }
        }
        break;

      case 2: // Disabled
        const imageToRemove = cardContent.querySelector('img');
        if (imageToRemove) {
  ...
  createChatMessage: async function _createChatMessage(title, msg, footer, titleImg, artImg = '', msgType = CONST.CHAT_MESSAGE_TYPES.OOC) {
    ...
    const animationSetting = game.settings.get(MODULE_NAME, 'chatAnimatedArtwork');
    let selectedArtwork;
    switch (animationSetting) {
      case 0: // Animated artwork enabled
        selectedArtwork = artImg;
        break;
      case 1: // Static artwork enabled
        selectedArtwork = titleImg;
        break;
      default:
        selectedArtwork = ''; // No artwork should be displayed
        break;
    }
  ...
  launchHelperPopup: async function _launchHelperPopup(imagePool = HELPER_GIFS) {
        ...
        render: () => {
          ...
          checkbox.change(async () => {
            if (checkbox[0].checked) {
              log('Changing Helper pop-up to disabled!', 'Helper');
              await game.settings.set(MODULE_NAME, 'showHelperPopup', false);
            } else {
              log('Changing Helper pop-up to enabled!', 'Helper');
  ...
  checkMandatoryModules: async function _checkMandatoryModules(mandatoryModules = MANDATORY_MODULES, logo = BOSS_LOOT_LOGO) {
    ...
    if (moduleMessage.length > 0) {
      ...
      await helperData.missingModulesCustomMessage('Warning', 'Missing Modules', moduleMessage, logo);
    }
  ...
  hideDivFromChatMessage: async function _hideDivFromChatMessage(cardId, className) {
    ...
    if (!elementToHide) {
      log(`Element with class "${className}" not found in ChatMessage with id [${cardId}]`, 'Helper', 'error');
      return;
    }

    elementToHide.classList.add('hidden');

    log(`Class hidden added near to "${className}" in ChatMessage with id [${cardId}]`, 'Helper');
  ...
  testCollision: function _testCollision(orig, dest, options) {
    if (!orig || !dest || !orig.x || !orig.y || !dest.x || !dest.y || !options) {
      log('Incorrect use of testCollision', 'Helper', 'error');
      return null;
    }
  ...
  /**
   * Retrieves the elevation of an animation in a sequencer.
   * @param {string|PersistentCanvasEffect} animation - The animation name or PersistentCanvasEffect instance.
   * @return {number|null} The elevation value of the animation or null if not found or invalid.
   */
  getAnimationElevation: function _getAnimationElevation(animation) {
    let seqAnimation;

    if (typeof animation === 'string') {
      const seqAnimationList = Sequencer.EffectManager.getEffects({ name: animation });
      if (seqAnimationList.length === 0) {
        log(`Cannot find Persistent Effect with name '${animation}'`, 'Helper', 'error');
        return null;
      } else if (seqAnimationList.length > 1) {
        log(`Found multiple Persistent Effects with name '${animation}'`, 'Helper', 'error');
        return null;
      }
      [seqAnimation] = seqAnimationList;
    } else if (animation && typeof animation === 'object' && 'data' in animation && 'elevation' in animation.data) {
      seqAnimation = animation;
    } else {
      log('Animation should be instance of String or PersistentCanvasEffect', 'Helper', 'error');
      return null;
    }

    return seqAnimation?.data?.elevation?.elevation ?? null;
  },
};
